...
class ToolRegistry:
    ...
    def register_skill_tool(self, skill_registry) -> None:
        """Register the load_skill tool and store SkillRegistry for delegation."""
        self._skill_registry = skill_registry

        def _execute(args: dict) -> str:
            return skill_registry.load_skill(args.get("name", ""))

        self.register_tool(Tool(
            name="load_skill",
            description=(
                "Load a skill by name to get specialized instructions for a task. "
                "Call this when the task matches a known skill's domain."
            ),
            parameters={
                "type": "object",
                "properties": {
            ...
            execute_fn=_execute,
            format_fn=lambda a: f"[skill] load_skill  {a.get('name', '')}",
            op_type=OpType.READ,
        ))

    def list_skills(self) -> list:
        registry = getattr(self, "_skill_registry", None)
        return registry.list_skills() if registry else []

    def get_skill_description(self, name: str) -> str | None:
        registry = getattr(self, "_skill_registry", None)
        skill = registry.get_skill(name) if registry else None
