...
class Agent:
    ...
    def __init__(self):
        ...
        self.tool_registry.register_skill_tool(SkillRegistry(Path(__file__).parent / ".skills"))
    ...
    def send_message(self, message):
        # Parse /{skill-name} trigger: /skill-name key1:val1 key2:val2 ...
        tokens = message.split()
        if tokens and tokens[0].startswith("/"):
            skill_name = tokens[0][1:]
            args = {k: v for t in tokens[1:] if ":" in t for k, v in [t.split(":", 1)]}
            result = self.tool_registry.execute_tool({"name": "load_skill", "args": {"name": skill_name}})
            if not result.startswith("Skill '"):
                self.conversation_history.append({"role": "system", "content": result})
                print(f"\033[1;32m[skill] loaded '{skill_name}'\033[0m")
            else:
                print(f"\033[1;33m[skill] {result}\033[0m")
            message = " ".join(f"{k}={v}" for k, v in args.items()) \
                if args else (self.tool_registry.get_skill_description(skill_name) or skill_name)
